...
// Self-contained icons to avoid external dependencies
const Icons = {
  Truck: () => (
...
// Global styles to ensure consistent rendering across different environments
const GlobalCardStyles = createGlobalStyle`
...
// Self-contained Toggle Button Component
const ToggleButton = ({ active = false, onClick = () => {}, size = 'medium' }) => {
  const handleClick = (e) => {
    e.stopPropagation();
    onClick();
  };
...
// Self-contained Video Component with fallback
const BackgroundVideo = ({ src, size = 160, opacity = 0.98, showVideo = true }) => {
  ...
  return (
    <div style={{ 
      ...
      width: size, 
      height: size, 
      opacity: videoLoaded ? opacity : 0.1,
...
// Main Embeddable Product Card Component
const EmbeddableProductCard = ({ 
  // Customization props for embedding
  width = '240px',
  height = 'auto',
  ...
  onPaymentMethodClick = null,
  shadow = 'default', // 'none' | 'subtle' | 'default' | 'strong'
  className = '',
  style = {},
  ...props
}) => {
  ...
  // Ensure client-side rendering for better SSR compatibility
  useEffect(() => {
    setIsClient(true);
  }, []);
  ...
  const paymentMethods = customPaymentMethods || defaultPaymentMethods;
  ...
  return (
    <>
      ...
      <StyledWrapper 
        ...
        language={language}
        shadow={shadow}
        className={`embeddable-product-card ${className}`}
        style={style}
      >
        <div className="card" style={{ backgroundColor, borderRadius }}>
                ...
                <div className="mini-payments">
                  <ul className="colors-container">
                    {paymentMethods.map((method) => (
                      <li 
                        key={method.id}
                        className={`payment-card ${method.id}`} 
                      ...
                      >
                        ...
                        <div className="label-text">{method.name}</div>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="image-container">
            ...
            <svg viewBox="0 0 1921 1081" xmlnsXlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" className="svg" aria-hidden="true">
              <defs>
                <radialGradient gradientUnits="objectBoundingBox" gradientTransform="translate(0.219) scale(0.563 1)" r="1.204" cy="0.5" cx="0.5" id={`radial-gradient-${Math.random()}`}>
                  <stop stopColor="#fff" offset={0} />
                  <stop stopColor="#bcbcbc" offset={1} />
                </radialGradient>
              </defs>
              <g transform="translate(-121.5 -92.5)">
                <rect fill={`url(#radial-gradient-${Math.random()})`} strokeWidth={1} strokeMiterlimit={10} stroke="#fff" transform="translate(122 93)" height={1080} width={1920} />
              </g>
            </svg>
            
            {/* Background video or fallback */}
            <BackgroundVideo 
              src="/assets/finance-anim.mp4" 
              size={160} 
              opacity={0.98} 
              showVideo={showVideo}
            />
...
const ToggleWrapper = styled.div`
  transform: scale(${props => props.size === 'small' ? '0.8' : props.size === 'large' ? '1.2' : '1'});
...
// Main Styled Component with comprehensive responsive design
const StyledWrapper = styled.div`
  ...
  isolation: isolate; /* Create new stacking context for embedding */
  ...
  .card {
    ...
    box-shadow: ${props => {
      switch (props.shadow) {
        ...
        case 'subtle': return 'rgba(100, 100, 111, 0.1) 0px 20px 15px -10px';
        case 'strong': return 'rgba(100, 100, 111, 0.4) 0px 60px 40px -30px';
        default: return 'rgba(100, 100, 111, 0.2) 0px 50px 30px -20px';
      }
    }};
  ...
  .card .content .brand { 
    font-weight: 900; 
    color: ${props => props.theme === 'dark' ? '#9ca3af' : '#a6a6a6'}; 
  ...
  .card .content .product-name { 
    font-weight: 700; 
    color: ${props => props.theme === 'dark' ? '#d1d5db' : '#666'}; 
    font-size: 0.7rem; 
  ...
  .card .content .color-size-container { 
    ...
    font-size: 0.7rem; 
    font-weight: 700; 
    color: ${props => props.theme === 'dark' ? '#9ca3af' : '#a8a8a8'}; 
    gap: 2rem; 
  ...
  .card .promo-row .promo-item { 
    ...
    align-items: center; 
    gap: 0.25rem; 
    background: ${props => props.theme === 'dark' ? 'rgba(255,255,255,0.1)' : '#ffffff'}; 
    border: 1px solid ${props => props.theme === 'dark' ? 'rgba(255,255,255,0.2)' : '#eef2ff'}; 
    border-radius: 999px; 
    padding: 2px 6px; 
...
export default EmbeddableProductCard;